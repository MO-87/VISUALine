...
def _materialize_runtime_config(
    ...
    Supports simple UI overrides:
        {"prompt": "person", "blur_intensity": 41}

    Also supports explicit node overrides:
        {"nodes.0.prompt": "person"}
        {"node.2.blur_intensity": 51}
    ...
    for key, value in overrides.items():
        ...
        # Simple syntax: prompt, blur_intensity, max_boxes, etc.
        ...
        if not applied:
            logger.warning(
                f"Override '{key}' did not match any existing node param in {base_config_path.name}."
            )
