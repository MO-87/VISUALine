...
// We only disable it if we detect WSL. Native Linux should try to use GPU.
...
// Support for Wayland (Hyprland, Sway, etc.)
if (process.platform === 'linux' && process.env.XDG_SESSION_TYPE === 'wayland') {
  console.log('Wayland detected, applying Ozone/Wayland flags...')
...
async function startPythonBackendIfNeeded() {
  ...
  if (app.isPackaged) {
    ...
    console.log(`[Electron] Using Python executable: ${pythonExecutable}`)

    // Ensure our patched vendor modules and source are in PYTHONPATH
    const vendorDinoPath = join(projectRoot, 'vendor', 'GroundingDINO')
    const srcPath = join(projectRoot, 'src')
    const currentPath = process.env.PYTHONPATH || ''
    const newPythonPath = currentPath 
      ? `${srcPath}:${vendorDinoPath}:${currentPath}`
      : `${srcPath}:${vendorDinoPath}`
