...
def warp(tenInput, tenFlow):
    ...
    if k not in backwarp_tenGrid:
        tensor_device = tenFlow.device
        tensor_dtype = tenFlow.dtype

        tenHorizontal = torch.linspace(-1.0, 1.0, tenFlow.shape[3], device=tensor_device, dtype=tensor_dtype).view(
            1, 1, 1, tenFlow.shape[3]).expand(tenFlow.shape[0], -1, tenFlow.shape[2], -1)
        
        tenVertical = torch.linspace(-1.0, 1.0, tenFlow.shape[2], device=tensor_device, dtype=tensor_dtype).view(
