...
from visualine.models.archs.scunet_wrapper import SCUNetArchWrapper

logger = logging.getLogger(__name__)

class SCUNetNode(NodeBase):
    use_torch: bool = True

    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        self.model_filename: str = self.config.get("model_filename", "scunet_color_real_gan.pth")
        self.fp16: bool = self.config.get("fp16", True)

        self.model_wrapper: SCUNetArchWrapper | None = None
    ...
    def setup(self, device: torch.device) -> None:
        ...
        model_cache_key = f"scunet_{self.model_filename}_fp16{self.fp16}"

        def model_loader():
            return SCUNetArchWrapper(
